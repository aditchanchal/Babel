...
CITIES = {
    ...
    'KANPUR': '26.450521,80.327390,30km'
}
...
def get_tweets(city, max_tweets):
    ...
    tweets = tw.Cursor(api.search, lang='en',
                       q='-filter:retweets', geocode=city).items(max_tweets)

    users_data = [[tweet.user.screen_name, tweet.user.location, tweet.user.followers_count,
                   tweet.user.friends_count, tweet.text, tweet.retweet_count, tweet.favorite_count,
                   tweet.user.listed_count, tweet.user.statuses_count,
                   len(tweet.entities["user_mentions"]), int(tweet.in_reply_to_status_id_str != None),
                   int(tweet.user.verified), len(tweet.entities["hashtags"]), (datetime.utcnow() - tweet.user.created_at).days,
                   int(tweet.retweet_count != 0), tweet.id] for tweet in tweets]
...
def create_dataframe(users_data):
    df = pd.DataFrame(data=users_data, columns=['user', 'location', 'followers', 'following', 'unfiltered_text', 'retweets',
                                                'likes', 'listed_count', 'tweets', 'mentions', 'isreply', 'verified',
                                                'hashtags', 'days', 'retweeted_before', 'url'])
    return df
...
def process():
    for city, location in CITIES.items():
        ...
        df['url'] = df['url'].apply(
            lambda x: 'https://twitter.com/statuses/' + str(x))
